...
void getLevels(ifstream &connections, ifstream &roots, ofstream &connectionsWithLevels)
{
	...
	while(!current.empty())
	{
		...
		while(!next.empty())
		{
			int cConn = next.front(); next.pop();
			// visited[currentConnection] = true;
			// level[currentConnection] = lvl;

			// cout<<"\nneighbours of "<<cConn<<endl;
			// getchar();
			for(int i=offset[cConn];i<offset[cConn+1];i++)
			{
				int currentOutgoing = dependency[i];
				// cout<<currentOutgoing<<" ";

				next.push(currentOutgoing);
				if(level[currentOutgoing] < level[cConn]+1){
					// next.push(currentOutgoing);
					level[currentOutgoing] = level[cConn]+1;
