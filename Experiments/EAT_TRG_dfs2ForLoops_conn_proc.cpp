...
public:
	...
	void dfs(int i, int &conn_proc_count);
	void printFPD();
...
void TRG::dfs(int i, int &conn_proc_count)
{
	int u, t, j,index,uPrime;
	int startTime, inTime;
	int count=0;
...
int main(int argc, char* argv[])
{
	...
	cout<<"Number of Queries:"<<T.sources.size()<<endl;
	for (unsigned int i = 0; i < T.sources.size(); i++)
	{
		...
		for (auto x : T.srcVertices)
		{
			...
			T.dfs(x.first,conn_proc_count);
			// fout<<c<<" "<<x.first<<" "<<conn_proc_count<<endl;
			c++;
		}
	...
	cout<<"Total connections processed:"<<conn_proc_count<<endl;
	cout<<"Average Number of Connections processed:"<<(conn_proc_count/T.sources.size())<<endl;
	fout<<"Average Number of Connections processed:"<<(conn_proc_count/T.sources.size())<<endl;
