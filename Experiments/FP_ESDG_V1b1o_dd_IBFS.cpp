...
public: 
	...
	void fastestPathDuration(int src, int *duration,int *best_st);
...
void esdg::fastestPathDuration(int src, int *duration, int *best_st)
{
	queue< int > q;
	...
	for(auto node=connSrc.begin();node<connSrc.end();node++)
	{
		...
		while(!q.empty())
		{
			...
			if(duration[cArray[currConnection].v]> cArray[currConnection].t+ cArray[currConnection].lambda - nStartTime[currConnection])
			{
				...
				best_st[cArray[currConnection].v] = nStartTime[currConnection];
			}
...
int main(int argc, char*argv[])
{
	...
	esdg G;
	int *fpd,*best_st;				// Array to store the fastest path duration from source to the respective vertex
	...
	for(int queryIndex=0; queryIndex<G.noOfQueries; ++queryIndex){
		...
		G.sortSrcVertices();
		start=clock();
		G.fastestPathDuration(G.src[queryIndex],fpd,best_st);
