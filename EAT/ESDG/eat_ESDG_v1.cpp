...
void initialization(esdg &E,connections &G,int s,int ts,int *&eat,queue<int> &q,vector<bool> &visited)
{
	for(int i=0;i<G.m;i++)
	{
...
void earliestArrivalTime(esdg &E,connections &G,int s,int ts,int *&eat,queue<int> &q,vector<bool> &visited)
{
	/*queue<int> q;
...
int main(int argc, char*argv[])
{
	...
	for(int i=0;i<Q.n;i++)
	{
		queue<int> q;
		vector<bool> visited(G.m,false);
		initialization(E,G,Q.sv[i],Q.t[i],eat,q,visited);
		start=clock();
		// earliestArrivalTime(E,G,Q.sv[i],Q.t[i],eat,count);
		earliestArrivalTime(E,G,Q.sv[i],Q.t[i],eat,q,visited);
		end=clock();
